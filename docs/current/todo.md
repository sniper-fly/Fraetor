...
- [ ] `src/intent_translation/domain/ports.py`  # `ScreenshotCapturePort`(capture)、`IntentTranslatorPort`(translate)
- [ ] `src/intent_translation/application/segment_screenshot_pairer.py`  # `SegmentScreenshotPairer`(`_pending`/`_ready`の状態機械、`on_speech_start`/`on_flush(produced_text)`/`pop_ready`/`reset`)
- [ ] `src/intent_translation/application/intent_translation_use_case.py`  # `IntentTranslationUseCase`(`ProofreadTextUseCase`と同型のフォールバック構造)
- [ ] `src/intent_translation/application/dictation_hook_adapter.py`  # `IntentTranslationDictationAdapter`(`SegmentLifecycleHookPort`/`RecognizedTextTransformPort`を実装し、`SegmentScreenshotPairer`と`IntentTranslationUseCase`を橋渡し)
- [ ] `src/intent_translation/infrastructure/screenshot/mss_screenshot_capturer.py`  # `MssScreenshotCapturer`(プラットフォーム分岐なしの単一実装、macOS環境でのみ動作検証、Linux対応はコード上のみ)
- [ ] `src/intent_translation/infrastructure/azure_openai_intent_translator.py`  # `AzureOpenAIIntentTranslator`(`AsyncAzureOpenAI`、`max_completion_tokens`使用、画像は`image_url`のdata URL形式)
- [ ] `tests/intent_translation/application/test_segment_screenshot_pairer.py`  # ON/OFF切替、`on_flush(produced_text=False)`で`_ready`に積まれないこと、FIFO順序、`reset`
- [ ] `tests/intent_translation/application/test_intent_translation_use_case.py`  # `translator=None`/`enabled=False`/`screenshot=None`/成功/例外・タイムアウト時のフォールバック
- [ ] `tests/intent_translation/application/test_dictation_hook_adapter.py`  # 委譲呼び出しの検証(pop_readyがNoneの場合は原文を返す等)
- [ ] `tests/intent_translation/infrastructure/screenshot/test_mss_screenshot_capturer.py`  # `@patch(".mss")`でモック、範囲外index、例外時`None`
- [ ] `tests/intent_translation/infrastructure/test_azure_openai_intent_translator.py`  # `max_completion_tokens`使用の検証、空テキスト即返し、`content`空時のフォールバック
...
- [ ] `src/shared/config/dynamic_settings.py`  # `intent_translation_enabled: bool = False`、`screenshot_monitor_index: int = Field(default=1, ge=0)`、`intent_translation_timeout_sec: int = Field(default=20, gt=0)`を追加
...
- [ ] `src/templates/index.html`  # 意図翻訳ON/OFFトグル(サーバー側`DynamicSettings.intent_translation_enabled`が真実源、`PUT /api/settings`を再利用)、`SETTINGS_FIELDS`に`screenshot_monitor_index`/`intent_translation_timeout_sec`を追加
- [ ] `pyproject.toml`  # `uv add openai` / `uv add mss`。`[[tool.mypy.overrides]]`に`module = "mss.*"`の`ignore_missing_imports = true`を追加
