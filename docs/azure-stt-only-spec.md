...
Linux 上で動作する音声入力アプリ。HTTP API で録音を開始/停止し、Azure STT でリアルタイム文字起こしを行い、ブラウザ UI にリアルタイム表示する。録音停止後、テキストをクリップボードにコピーし、オプションでアクティブウィンドウへ自動ペーストする。
...
| POST | `/api/paste-toggle` | ペースト ON/OFF トグル |
| GET | `/api/paste-status` | ペースト設定の現在値を取得 |
| GET | `/api/history` | 履歴一覧を取得 |
| POST | `/api/finalize-session` | ペースト OFF 時の最終テキスト送信 |
...
| `session_end` | `{"paste_enabled": false}` or `{}` | セッション終了通知 |
| `keepalive` | `""` | 接続維持 |
| `error` | `{"message": "..."}` | エラー通知 |
...
### POST /api/paste-toggle

- レスポンス: `{"paste_enabled": bool}`
- 次回セッションから反映
...
- レスポンス: `{"ok": bool}`
- ペースト OFF 時にブラウザから編集済みテキストを受け取りクリップボードにコピー
...
[停止]
  POST /api/toggle-recording (recording=true → false)
    ...
    5. ペースト ON: copy_and_paste → JSONL 保存
       ペースト OFF: pending_session 設定 → session_end 送信
...
class AppState:
    ...
    recording: bool                                  # 録音状態
    paste_enabled: bool                              # ペースト設定
    pending_session: Session | None                  # ペースト OFF 時の保留セッション
```
