...
class TestPyperclipClipboard:
    async def test_calls_pyperclip_copy_with_text(self) -> None:
        ...
        with patch(
            "src.transcript_history.infrastructure.pyperclip_clipboard.pyperclip.copy"
        ) as mock_copy:
            await clipboard.copy("テスト文章")
